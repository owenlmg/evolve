 ...
 * @abstract 
 */
class Erp_Model_Sale_Invoice extends Application_Model_Db
{
    /**
     * 表名、主键
     */
    protected $_name = 'erp_sale_invoice';
    protected $_primary = 'id';
    
    public function getNewNum()
    {
        $num_pre = 'INVS'.date('ymd');
    ...
    public function getData($condition = array(), $invoice_id = null)
    {
        $sql = $this->select()
                    ...
                    ->joinLeft(array('t9' => $this->_dbprefix.'bpartner'), "t1.customer_id = t9.id", array('customer_code' => 'code', 'customer_ename' => new Zend_Db_Expr("case when t9.ename = '' then t9.cname else t9.ename end"), 'customer_cname' => 'cname', 'customer_bank_type' => 'bank_type', 'customer_bank_account' => 'bank_account', 'customer_tax_id' => 'tax_id', 'customer_tax_num' => 'tax_num', 'customer_bank_payment_days' => 'bank_payment_days'))
        ...
        if($invoice_id){
            ...
            for($i = 0; $i < count($data); $i++){
                ...
                if($data[$i]['state'] != 1){
                    // 获取审核情况
                    $review_state = $help->getReviewState('sale_invoice_add', $data[$i]['id']);
                    /* echo '<pre>';
                ...
                if($help->chkIsReviewer('sale_invoice_add', $data[$i]['id'], $employee_id)){
