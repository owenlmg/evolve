...
var itemsStore = Ext.create('Ext.data.Store', {
    ...
    proxy: {
        type: 'ajax',
        reader: 'json',
        url: homePath+'/public/erp/stock_out/getoutstockitems'
    }
});
...
var itemsRowEditing = Ext.create('Ext.grid.plugin.CellEditing', {
    ...
    listeners: {
    	beforeedit: function(editor, e){
    		if(Ext.getCmp('outstockSaveBtn').isDisabled()){
    			return false;
    		}
    	}
    }
...
function printOrder(){
	...
	if(id != null){
	    ...
	    Ext.Ajax.request({
	        url: homePath + '/public/erp/stock_out/getprint/id/' + id,
	        params: '',
	        method: 'POST',
...
var importWin = Ext.create('Ext.window.Window', {
    ...
    items: [Ext.create('Ext.form.Panel', {
        id: 'importForm',
    	border: 0,
        url: homePath+'/public/erp/stock_in/importitems/type/out',
        bodyPadding: '2 2 0',
        fieldDefaults: {
        	msgTarget: 'side',
            labelAlign: 'right',
...
var itemsGrid = Ext.create('Ext.grid.Panel', {
    ...
    columns: [{
        ...
        align: 'center',
        editor: 'numberfield',
        ditor: new Ext.form.field.Number({
        	listeners: {
            	change: function( sel, newValue, oldValue, eOpts ){
            		var selection = Ext.getCmp('itemsGrid').getView().getSelectionModel().getSelection();
...
var itemsForm = Ext.create('Ext.form.Panel', {
	id: 'itemsForm',
	border: 0,
    url: homePath+'/public/erp/stock_out/editoutstock',
    bodyPadding: '2 2 0',
    fieldDefaults: {
        msgTarget: 'side',
        labelWidth: 75
        ...
        handler: function() {
            ...
            if(form.isValid()){
            	...
            	if(itemsStore.getCount() > 0){
                                        ...
                                        if(data.success){
                                            ...
                                            if(data.outstock_id){
                                                ...
                                                //if(changeRowCnt > 0){
                                                    ...
                                                    Ext.Ajax.request({
                                                        url: homePath+'/public/erp/stock_out/edititems',
                                                        params: {json: json},
                                                        method: 'POST',
                                                        success: function(response, options) {
                                                            var data = Ext.JSON.decode(response.responseText);
                                            ...
                                            if(errInfo != ''){
                                                ...
                                                Ext.getCmp('outstockGrid').getSelectionModel().clearSelections();
                                            }
                                        }else{
                                            Ext.MessageBox.alert('错误', data.info);
                                        }
            		...
            		Ext.MessageBox.alert('错误', '请添加发货项目！');
            	}
            }
        }
    },{
...
// 发货单窗口
var outstockWin = Ext.create('Ext.window.Window', {
	title: '库存交易-发货',
	id: 'outstockWin',
	width: 900,
	modal: true,
