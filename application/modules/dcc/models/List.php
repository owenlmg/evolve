 ...
 * @abstract
 */
class Dcc_Model_List extends Application_Model_Db {

    /**
     * 表名、主键
     */
    protected $_name = 'doc_file_list';
    ...
    public function getDataById($id) {
        $sql = $this->select()
                ->setIntegrityCheck(false)
                ->from(array('t1' => $this->_name), array('id', 'code', 'ver', 'file_ids', 'name', 'state', 'create_user', 'update_user'))
    ...
    public function getMy($type, $where, $myId, $file_ids, $start, $limit) {
            ...
            $sql1 = $this->select()
                    ...
                    ->where("t1.state != 'Deleted' and t1.del_flg = 0 and (t1.create_user = $myId or t1.update_user = $myId)")
                    ->where($where);
            // 我审核过的
            $sql2 = $this->select()
                    ->setIntegrityCheck(false)
                    ...
                    ->joinLeft(array('t5' => $this->_dbprefix . 'product_catalog'), "t0.project_no = t5.id", array('project_name' => 'model_internal'))
                    ->joinLeft(array('t8' => $this->_dbprefix . 'codemaster'), "t.category = t8.code and t8.type=4", array('category' => 'id', 'category_name' => 'text'))
                    ->where("t1.state != 'Deleted' and t1.del_flg = 0")
                    ->where($where);
            // 我将要审核的
            $sql3 = $this->select()
                    ->setIntegrityCheck(false)
                    ...
                    ->where("t1.state = 'Reviewing' and (t4.actual_user is null or !(FIND_IN_SET($myId,t4.actual_user))) and (FIND_IN_SET($myId, t4.plan_user))")
                    ->where($where);
            // 共享给我的
            $sql4 = $this->select()
                    ->setIntegrityCheck(false)
                    ...
                    ->where($where);
            if(count($file_ids) > 0) {
                $sql4 = $sql4->where("t1.id in (" . implode(',', $file_ids) . ")");
            }
        if($type == 1) {
            $sqlArray = array($sql1);
        } else if($type == 2) {
