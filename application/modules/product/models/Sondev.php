 ...
 * @abstract
 */
class Product_Model_Sondev extends Application_Model_Db
{
    /**
     * 表名、主键
     */
    protected $_name = 'product_bom_son_dev';
    ...
    public function getListByRecordkey($recordkey){
        $sql = $this->select()
                    ->setIntegrityCheck(false)
                    ->from(array('t1' => $this->_name))
                    ->joinLeft(array('t2' => $this->_dbprefix.'product_materiel'), "t1.id = t2.id", array('state', 'description'))
